...
if($Folder.count -eq 0)
{
    ...
    $R=Robocopy $OldVersion.FullName $NewVersion.FullName /E
...
$FunctionList = Get-ChildItem -Path ($PSScriptRoot+"\"+$Version+"\Functions\Public" )|Where-Object {$_.Name -like "*ps1"} |Select-Object @{n='Function';e={$_.Name -replace ".ps1",""}}
Write-Verbose "List of Functions: " 
Write-Verbose ($FunctionList|FT|Out-String)
